...
class TestReflector(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_reflection_failure(self):
        gem = await self.reflector.reflect_on_episode("ep-001", success=False)
        self.assertIsNotNone(gem)
        self.assertEqual(gem.title, "Pre-action Validation Needed")
        self.assertEqual(gem.status, "PROPOSED")

    async def test_reflection_feedback(self):
        gem = await self.reflector.reflect_on_episode("ep-002", success=True, feedback="I prefer shorter summaries")
        self.assertIsNotNone(gem)
