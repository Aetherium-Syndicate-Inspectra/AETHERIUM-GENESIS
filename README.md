...
The diagram below is now aligned to the current data architecture (logical storage model + runtime data flows): Akashic ledger chain, derived memory projections, governance/execution events, and entropy treasury records.

แผนภาพด้านล่างอ้างอิงโครงสร้างข้อมูลจริง (logical storage model + runtime data flows): Akashic ledger chain, memory projections, governance/execution events และ entropy treasury records
...
erDiagram
    ...
    INTENT_EVENT ||--|| GOVERNANCE_DECISION : "evaluated by"
    GOVERNANCE_DECISION ||--o| EXECUTION_OUTCOME : "allows"
    INTENT_EVENT ||--o{ AKASHIC_BLOCK : "committed as"
    GOVERNANCE_DECISION ||--o{ AKASHIC_BLOCK : "appends policy event"
    EXECUTION_OUTCOME ||--o{ AKASHIC_BLOCK : "appends outcome event"
    AKASHIC_BLOCK ||--o{ MEMORY_PROJECTION : "projects to"
    AKASHIC_BLOCK ||--o{ GEM_STATE : "adopts/updates"
    INTENT_EVENT ||--o{ ENTROPY_LEDGER : "assessed for QoU"
```

### Data source mapping / แหล่งอ้างอิงโครงสร้างข้อมูล

- **Akashic block chain**: `data/akashic_records.json`, managed by `src/backend/genesis_core/memory/akashic.py`.
- **Memory projections**: `data/memory/*`, generated by `src/backend/memory/fabric.py`.
- **Gem projection state**: `data/derived_gems.json`, maintained by `src/backend/genesis_core/memory/akashic.py` (`MemoryProjectionManager`).
- **Entropy ledger logical table**: documented in `docs/ENTROPY_ECONOMY_TECHNICAL_SPEC_TH.md`.
...
## 💡 Proposed Next Functions / ข้อเสนอฟังก์ชันและแนวทางต่อยอดใหม่

> Curated backlog only (active proposals). Completed items are intentionally excluded from both English and Thai lists.
>
> รายการนี้เป็น backlog ที่ยังดำเนินการได้เท่านั้น (ไม่นำรายการที่ทำเสร็จแล้วมาปะปนทั้งภาษาอังกฤษและภาษาไทย)

### English

1. **Governance Replay Console** — replay any `trace_id` and visualize policy decisions, approvals, and execution outcomes in a single timeline.
2. **Policy Drift Detector** — compare current policy decisions vs historical baselines and flag high-risk drift before production rollout.
3. **Memory Integrity Sentinel** — periodic hash-chain validation + anomaly alerting for Akashic and entropy ledgers.
4. **Cross-Vessel Transaction Envelope** — idempotent multi-step execution contract with compensation primitives and rollback checkpoints.
5. **Directive Provenance Explorer** — query directive envelopes by correlation/causation and inspect full provenance graph for audits.

### ภาษาไทย

1. **Governance Replay Console** — เครื่องมือ replay ตาม `trace_id` เพื่อดูลำดับ policy, approval และผล execution ใน timeline เดียว
2. **Policy Drift Detector** — เปรียบเทียบผลตัดสิน policy ปัจจุบันกับ baseline เดิม แล้วแจ้งเตือนความเสี่ยงที่เบี่ยงเบนสูง
3. **Memory Integrity Sentinel** — งานตรวจ hash-chain อัตโนมัติพร้อมแจ้งเตือนความผิดปกติของ Akashic/entropy ledger
4. **Cross-Vessel Transaction Envelope** — มาตรฐานงานข้าม vessel แบบ idempotent พร้อม compensation และจุด rollback
5. **Directive Provenance Explorer** — เครื่องมือค้น envelope จาก correlation/causation และตรวจกราฟ provenance แบบ audit-ready
