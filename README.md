...
The diagram below reflects the current codebase and the persisted data structure that supports the runtime: ingress/runtime state, approval state, append-only Akashic records, entropy continuity, and manifestation projections.

แผนภาพด้านล่างสะท้อนทั้งโค้ดปัจจุบันและโครงสร้างข้อมูลที่ถูกจัดเก็บจริงเพื่อรองรับ runtime ได้แก่ ingress/runtime state, approval state, Akashic records แบบ append-only, entropy continuity และ projection สำหรับการแสดงผล
...
flowchart TD
    ...
    runtime[Governed Runtime<br/>policy evaluation + approval routing]
    approvals[(Approval State<br/>PENDING / APPROVED / REJECTED / DENIED)]
    vessels[Execution Vessels<br/>workspace / adapters]
    akashic[(Akashic Records<br/>append-only audit chain)]
    entropy[(Entropy Ledger<br/>hash-chain continuity)]
    projections[(Memory Projections<br/>episodes / gems / audit views)]
    body[Manifestation Layer<br/>dashboard / public / sandbox UI]

    intent --> ingress --> envelope --> runtime
    runtime -->|PENDING_APPROVAL| approvals
    runtime -->|DENIED / REJECTED audit| akashic
